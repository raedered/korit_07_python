...
1. 예외 처리의 필욜성
    1) 예외(exception) : 개발자가 직접 처리 할수 있는 문제
...
어떤 값이든 0으로 나눌 수 없기 때문에 if a == 0 을 통해 0으로 나누는 것을 아예 시도 할 수 없도록 예외처리를 함.
...
|  5   |      E0Error       | 파일에서 더 이상 읽어 들일 데이터가 없을 때 |
|  6   | ModulNotFoundError |            import할 모듈이 없을때           |
|  7   | FileNotFoundError  |           존재하지 않는 파일일 때           |
|  8   |     IndexError     |          잘못된 인덱스를 사용할 때          |
|  9   |     NameError      |        잘못된 이름(변수)을 사용할 때        |
|  0   |    SyntaxError     |               문법이 틀렸을 때              |
|  11  |     TypeError      |   계산할려는 데이터의 유형이 잘못되었을 때  |
|  12  |     ValueError     |    계산할려는 데이터의 값이 잘못되었을때    |
+------+--------------------+---------------------------------------------+
'''
table = PrettyTable()
table.field_names = ['이름', '예외 클래스', '의미']
exception = [
    (1, 'BaseException', '최상위 예외 클래스'),
    (2, 'Exception', '대부분 예외 클래스의 슈퍼 클래스'),
    (3, 'ArithmeticError', '산술 연산에 문제가 있을 때'),
...
try / except 문을 사용하여 '예외가 발생햇습니다.'를 출력할 수 있도록 작상하세요.
'''
# try :
...
'''
    1) 특정 에외만 처리하는 방식
        try / except 문을 사용하면 기본적으로 예외의 종류에 상관없이 모든 예외가 처리됨. 하지만 이상에서 본 것처럼, 0으로 나누는 경우와, str 자료형을 시수로 바꾸고자 하는 경우에 서로 다른 메시지를 출력해줄 수 있다면, 개발자에게 예외를 처리할 수 있을만한 ㄹ추가적인 정보를 제공할 수 있음
...
Java에서와 동이할게 Exception이 가장 상위에 있게 되면 모든 예외가 전부 Exception으로 잡히기 때문에 순서 역시 중요합니다.

3. 예외 메시지 처리하기
    이상까지는 특정 예외가 발생했을 때 저희가 메시지를 커스텀해서 사용했지만, 기본적으로 이미 예외 메시지를 기지고 있는 경우도 있습니다. default exception message 를 출력하는 방식 학습할겁니다.
...
'''
4. else / finally
    try / except 문에 else / finlly를 달수 있는데,
    else : 예외가 발생하지 않으면 처리되는 구문
    finally : 예외 발생 여부과 관계 없이 맨 마지막에 항상 처리되는 구문 
'''
# try :
#     a = int(input('나누는 수를 정수로 입력하세요 >>> '))
#     b = int(input('나누어지는 수를 정수로 입력하세요 >>> '))
#     result = b / a          # 예외가 발생할 수 있는 구간이 try 문 내에 있어야 만 합니다.
...
a = 0
b = 아무 정수
를 넣었을 때 ZeroDivisionError가 예외 처리되지 않고 오류 발생을 하는것을 확인할수 있습니다.
'''

'''
5. 강제로 예외를 발생시키기
    어떤 사람이 나이를 정수로 입력받는 프로그램을 사용한다고 가정했을 때,
    컴푸터 상으로는(그리고 파이썬 상으로는) -1000이 정수이기 때문에 예외가 발생하지 않습니다(그래서 우리는 0미만 200초과를 조건문으로 처리하는 것을 학습했었습니다.)
    하지만 -1000이 되는 것이 불가능하기 때문에 조건문이 아니라 직접 예외를 발생시켜서 처리하는 방법을 학습합니다. -> raise 문
...
raise의 경우 강제로 예외를 발생시킨다는 점에서 주로 사용되는예외 클래스는 Exception입니다.
'''
...
'''
과제:
    나이가 200살 초과일때 TooManyAgeError 예외를 발생시켜서 '200살 초과된 나이는 입력할수 없습니다.'라는 메시지를 출력할 수 있도록 이상의 코드를 수정하시오
'''
'''
과제:
사용자의 점수를 0 이상 100 이하로 입력 받아서 점수가 80점 이상이면 합격, 아니면 불합격을 출력하는
프로그램을 작성하는데, 0이상 100이하입니다.'라는 예외 메시지를 출력하시오 -> 사용자 정의 예외 클래스를 정의해야하겠죠.
ScoreOutOfRangeError 플래스를 정의해서 사용하겠습니다.

입력 받는데 예를 들어 백점 이라고 입력하면 '점수는 숫자로 입력해야합니다.'라는 메시지를 출력하세요.
...
#     if score > 100 :
#         raise ScoreOutOfRangeError('0이상 100이하입니다.')
#     score = int(score)
#     if score >= 80:
#         print('합격')
#     else:
...
만약 사용자가 0을 입력하거나 숫자가 아닌 값을 입력하면 예외 메시지를 출력하시오

지시 사항
1. 사용자로부터 숫자를 입력받는다.
2. 입력 받은 숫자로 100을 나누어 결과를 출력한다.
3. 입력 값이 -일 경우 적절한 예외를 처리하여 '0으로 나눌 수 없습니다.'라는 메시지를 출력한다.
...
6. 에외가 발생하지 않는 경우 "정상적으로 처리되었습니다." 라는 메시지와 함께 인덱스의 값을 출력한다.
7. 프로그램 종료 메시지를 출력한다.
8. 마이너스인덱스는 적용시키지 않는다. -> 사용자 정의 예외 클래스를 통해서 적용합니다.
    -> NegaviceIndexError라고 이름 짓고 처리하겠습니다.
'''
# class NegaviceIndexerror(Exception) :
#     pass
# my_list = [10, 20, 30, 40, 50]
#
# try :
#     list_index = int(input('사용할 인덱스 번호를 입력하세요 >>> '))
#     result = my_list[list_index]
#     if list_index < 0 :
#         raise NegaviceIndexerror('마이너스 인덱스는 입력할수 없습니다.')
#     print(result)
# except NegaviceIndexerror as e :
#     print(e)
# except IndexError as e :
#     print(f'인덱스 범위를 벗어났습니다. {e}')
# except ValueError as e :
#     print(f'0 ~ 100 까지 숫자를 입력하세요 {e}')
# except Exception as e :
#     print(f'예상할 수 없는 예외가 발생햇습니다. {e}')
# else :
#     print('정상적으로 처리되었습니다.')
# finally:
#     print('프로그램 종료')
...
print(vars(person1))                # vars(객체명) : 객체명 - 값을 dictionary로 만들어줍니다. JSON이 생각나죠.
print(getattr(person1, 'age'))      # 결과값 : 21
# getattr()의 두 번째 argument는 인스턴스 변수명을 받습니다. -> 그 데이터를 str로 받습니다.
print(getattr(person1, 'name'))     # 결과값 : 김일

# attr_name = input('속성명을 입력해주세요 >>> ')
# print(getattr(person1, a)) # 클래스 변수인 school이

# try :
#     str = input('속성명을 입력해주세요 >>> ')
#     print(getattr(person1, str))
