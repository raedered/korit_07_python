...
# 이상의 경우만 작성했을 때 모니터가 깜빡이는 것을 확인할 수 있는데 , 이는 코드가 다 돌아가면 프로그램이 종료되기 때문에, 창이 켜젓ㅆ다가 꺼지는 것입니다.
...
# 금데 도형 그릴 때마다 반복문 쓰는거 너무짜증나니까 그냥 함수를 정의합시다.
def draw_shape(n):
    for _ in range(n) :
        t.forward(100)
        t.left(360/n)
...
def draw_spirograph(size_of_gap):
    for _ in range(360 // size_of_gap):         # // 나누기의  몫 연산자
        t.color(random.choice(colors))
        t.circle(100)
...
draw_spirograph(30)
# 이상의 코드에서의 문제점은
# 1. 매개변수인 size_of_gap은 n 번째 원과 n+1 번쩨 원의 각도차이를 나타내는 것 같은데,
