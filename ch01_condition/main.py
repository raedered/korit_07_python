'''
1. if문
    if문은 조건이 참일 대만 해당 블록의 코드 실행
2. if - else 문
    if는 조건이 참일 대 실행 / false 일때는 else 부분 실행
3. if - elif - else 문
'''
# age = 21
# if age > 20:
...
# input()의 결과값의 자료형은 str이고 그걸 다시 int로 바꾸는 함수가 적용되어 일종의 chaining method처럼 쓸수 있겠네요
# print(f'당싱의 나이는 {age}살이고, 내년에는 {age + 1}살이 됩니다.')
...
논리 연산자 :
    ...
    3) not : if not A -> A가 false일 대 실행문 실행             !
'''