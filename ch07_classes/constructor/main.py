'''
아까 전에는 일종의 setter를 활용하여 속성에 속성값을 넣어 줬습니다.
그럼 Java에서 수업한 것처럼 속성값이 대입되지 않는 객체를 생성한 다음에 속성값을 집어넣어주는 과정을 거쳐야 합니다.
...
Java / JS 등에서는 생성자 명은 클래스명과 동일하거나 constructor인데, 또 python만 지 한자서 이상한걸로 생성자를 만듭니다.
'''
class Candy2():
...
4. 이스턴스가 삭제되면 다음과 같은 메시지를 출력할 수있도록 작성하시오
james is dead.
'''
# class Person :
...
class Student :
    ...
    def show_info(self):
        print(f'{self.name} 학생의 나이는 {self.age} 살로, 파이썬 과목의 점수는 {self.score}점 입니다.')
...
그렇다면 Java를 기주능로 봤을 때 setter 내부에는 비지니스 로직이 들어갈 수 있었습니다.
