...
'''
1. 서브 클래스의 __init__()
    서브클래스는 슈퍼 클래스가 없으면 존재할 수 없습니다. 그래서 서브 클래스의 생성자를 구현할 때는 '반드시 슈퍼 클래스의 생성자를 먼저 호출'하는 코드를 작성해야만 합니다.
    ...
    이후에 슈퍼 클래스에서 생성된 인스턴스 변수인 name이 서브 클래스로 전달되고, 이후에 서브 클래스에서 school인스턴스 변수를 선언 및 초기화하여 저장하면서 서브 클래스의 인스턴스가 생성됩니다.
...
2. 서브 클래스의 인스턴스 자료형
    ...
    Student 클래스의 객체는 Student의 인스턴스이면tj Person의 인스턴스
...
2. 서브 클래스 Hybrid는 다음과 같은 특징을 지니고 있습니다.
    ...
    3) 현제 주유량과 충전량을 모두 확인할 수 있는 hybrid_info() 메서드가 있습니다.
...
#         if oil <= 0:                    # 매개변수 oil이 이하라면
#             return                      # 메서드 종료
#         self.oil += oil
#         if self.oil > Car.max_oil:      # 근데 self.oil의 현제 값이 50 chrhkfkaus
...
#         super().__init__(oil)           # 부모클래스의 __init__을 가지고 온다
#         self.amount = amount
#         print('하이브리드 차량이 생산되었습니다.')
#
#     # 부모 클래스의 메서드를 오버라이드
#     def add_oil(self, oil):
#         super().add_oil(oil)            # 부모클래스의 add_oil 메서드를 가지고 온다.
...
circle = Circle('원', 5)
circle.draw()
print(f'원의 넓이 : {rectangle.area()}')

실행 예
반지름이 5인 원1이 생성되었습니다.
이름이 원인 원의 넓이는 ___ 입니다.
원의 넓이 : ____입니다.
너비가 10, 높이가 8인 직사각형이 생성 되었습니다.
...
class Shape :
    ...
    def draw(self):
        print(f'이름이 {self.name}이 생성되었습니다.')
...
class Rectangle(Shape) :
    ...
    def draw(self):
        print(f'이름이 {self.name}인 직사각형의 넓이는 {self.rectangle()}입니다.') # override를 받이는 했지만 부모 부분은 사용하지 않음
...
rectangle.draw()
print(f'직사각형의 넓이 : {rectangle.rectangle()}')